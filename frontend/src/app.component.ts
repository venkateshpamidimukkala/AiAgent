...
export class AppComponent implements OnInit, OnDestroy {
  ...
  jiraConnected=false; jiraConfigured=false; jiraUrl=''; jiraEmail=''; jiraToken=''; jiraQuery=''; jiraBoard=''; jiraIssues:JiraIssue[]=[]; jiraLoading=false;
  inboxSource='all'; jiraCommentIssue?:JiraIssue; jiraComment=''; jiraComments:JiraComment[]=[]; jiraReplyTo?:JiraComment; jiraCommentsLoading=false;
  ...
  searchJira(){const query=this.jiraQuery.trim();const board=this.jiraBoard.trim();if(!query&&!board){this.status='Enter a Jira board name or issue search.';return;}if(!this.jiraConnected){this.status='Connect Jira before searching issues.';return;}this.jiraLoading=true;this.http.post<{issues:JiraIssue[]}>('/api/jira/search',{query,board_name:board}).subscribe({next:v=>{this.jiraIssues=v.issues;this.jiraLoading=false;this.status=v.issues.length?`Found ${v.issues.length} Jira issues`:'No matching Jira issues found';},error:e=>{this.jiraLoading=false;this.status=e.error?.detail||'Unable to search Jira';}});}
  ...
  addJiraComment(){if(!this.jiraCommentIssue||!this.jiraComment.trim())return;const issue=this.jiraCommentIssue;this.http.post<{status:string}>('/api/jira/comment',{issue_key:issue.key,body:this.jiraComment.trim(),reply_to:this.jiraReplyTo?.id||null}).subscribe({next:()=>{this.status=`Comment added to ${issue.key}`;this.jiraCommentIssue=undefined;this.jiraReplyTo=undefined;this.jiraComment='';this.loadJiraComments(issue);},error:e=>this.status=e.error?.detail||'Unable to add Jira comment'});}
