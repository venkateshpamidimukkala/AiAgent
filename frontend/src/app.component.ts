...
export class AppComponent implements OnInit, OnDestroy {
  ...
  jiraVoiceState:'idle'|'listening'|'processing'|'ready'|'posted'='idle'; jiraVoiceTranscript=''; jiraVoiceDraft=''; jiraVoiceError=''; jiraVoiceDuration=0; jiraVoicePostedAt?:Date; jiraVoicePostedPreview=''; private jiraVoiceRecognition:any; private jiraVoiceFinalTranscript=''; private jiraVoiceTimer?:ReturnType<typeof setInterval>;
  ...
  currentTime=''; private clockTimer?: ReturnType<typeof setInterval>; private jiraSyncTimer?: ReturnType<typeof setInterval>; private voicesChangedHandler?:()=>void;
  ...
  ngOnDestroy(){if(this.clockTimer)clearInterval(this.clockTimer);if(this.jiraSyncTimer)clearInterval(this.jiraSyncTimer);if(this.voicesChangedHandler&&'speechSynthesis' in window)speechSynthesis.removeEventListener('voiceschanged',this.voicesChangedHandler);this.stopSpeaking();}
  ...
  openMockInbox(source:'outlook'|'teams'){
    ...
    this.status=`Showing mock ${source === 'outlook' ? 'Outlook' : 'Teams'} messages.`;
  }
  ...
  loadJiraIssue(issueKey:string){if(!this.jiraConnected)return;this.jiraLoading=true;this.jiraSyncing=true;this.http.get<JiraIssue>(`/api/jira/issues/${encodeURIComponent(issueKey)}`).subscribe({next:issue=>{this.jiraIssues=[issue,...this.jiraIssues.filter(item=>item.key!==issue.key)];this.jiraSelectedIssue=issue;this.jiraCommentIssue=issue;this.jiraLastSynced=new Date().toISOString();this.jiraLoading=false;this.jiraSyncing=false;this.loadJiraComments(issue);this.status=`Loaded ${issue.key} from Jira`;},error:e=>{this.jiraLoading=false;this.jiraSyncing=false;this.status=e.error?.detail||`Unable to load ${issueKey} from Jira`;}});}
  selectJiraIssue(issue:JiraIssue){this.jiraSelectedIssue=issue;this.jiraCommentIssue=issue;this.loadJiraComments(issue);}
  prepareJiraDailyComment(issue:JiraDailyIssue){this.jiraSelectedIssue=issue;this.jiraCommentIssue=issue;this.jiraReplyTo=undefined;this.jiraComment='';this.jiraVoiceState='idle';this.jiraVoiceTranscript='';this.jiraVoiceDraft='';this.jiraVoiceError='';this.jiraVoicePostedAt=undefined;this.loadJiraComments(issue);setTimeout(()=>document.getElementById(`jira-voice-${issue.key}`)?.focus());}
  startJiraVoice(issue:JiraDailyIssue){this.prepareJiraDailyComment(issue);const Recognition=(window as any).SpeechRecognition||(window as any).webkitSpeechRecognition;if(!Recognition){this.jiraVoiceError='Voice recording is not supported in this browser. You can still write an update below.';this.status=this.jiraVoiceError;return;}this.jiraVoiceState='listening';this.jiraVoiceDuration=0;this.jiraVoiceFinalTranscript='';this.jiraVoiceTranscript='';this.jiraVoiceError='';this.jiraVoiceTimer=setInterval(()=>this.jiraVoiceDuration++,1000);const recognition=new Recognition();this.jiraVoiceRecognition=recognition;recognition.lang=this.inputLanguage||'en-US';recognition.continuous=true;recognition.interimResults=true;recognition.onresult=(event:any)=>{let interim='';for(let i=event.resultIndex;i<event.results.length;i++){const text=event.results[i][0].transcript;if(event.results[i].isFinal)this.jiraVoiceFinalTranscript+=text+' ';else interim+=text;}this.jiraVoiceTranscript=(this.jiraVoiceFinalTranscript+interim).trim();this.jiraComment=this.jiraVoiceTranscript;};recognition.onerror=()=>{this.stopJiraVoice();this.jiraVoiceError='We could not hear that update. Try again or type the update instead.';};recognition.onend=()=>{if(this.jiraVoiceState==='listening')this.stopJiraVoice();};recognition.start();}
  stopJiraVoice(){if(this.jiraVoiceTimer){clearInterval(this.jiraVoiceTimer);this.jiraVoiceTimer=undefined;}if(this.jiraVoiceRecognition){this.jiraVoiceRecognition.stop();this.jiraVoiceRecognition=undefined;}if(!this.jiraVoiceTranscript.trim()){this.jiraVoiceState='idle';return;}this.jiraVoiceState='processing';this.jiraVoiceDraft='';setTimeout(()=>{this.jiraVoiceDraft=this.buildJiraVoiceUpdate(this.jiraVoiceTranscript);this.jiraComment=this.jiraVoiceDraft;this.jiraVoiceState='ready';},650);}
